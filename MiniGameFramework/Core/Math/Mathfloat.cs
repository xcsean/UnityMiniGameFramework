...
{
    public partial struct MathfloatInternal
    {
    ...
    public partial struct Mathfloat
    {
        ...
        public static readonly float Epsilon =
            MathfloatInternal.IsFlushToZeroEnabled ? MathfloatInternal.FloatMinNormal
            : MathfloatInternal.FloatMinDenormal;
        ...
        {
            if (Mathfloat.Abs(target - current) <= maxDelta)
                return target;
            return current + Mathfloat.Sign(target - current) * maxDelta;
        }
        ...
        public static float SmoothDamp(float current, float target, ref float currentVelocity, float smoothTime, float maxSpeed, float deltaTime)
        {
            ...
            change = Mathfloat.Clamp(change, -maxChange, maxChange);
        ...
        {
            return Clamp(t - Mathfloat.Floor(t / length) * length, 0.0f, length);
        }
        ...
        public static float PingPong(float t, float length)
        {
            ...
            return length - Mathfloat.Abs(t - length);
        }
        ...
        {
            float delta = Mathfloat.Repeat((target - current), 360.0F);
            if (delta > 180.0F)
                delta -= 360.0F;
            return delta;
        }
        ...
        internal static float RoundToMultipleOf(float value, float roundingValue)
        {
            if (roundingValue == 0)
                return value;
            return Mathfloat.Round(value / roundingValue) * roundingValue;
        }
        ...
        internal static float GetClosestPowerOfTen(float positiveNumber)
        {
            if (positiveNumber <= 0)
                return 1;
            return Mathfloat.Pow(10, Mathfloat.RoundToInt(Mathfloat.Log10(positiveNumber)));
        }

        internal static int GetNumberOfDecimalsForMinimumDifference(float minDifference)
        {
            return Mathfloat.Clamp(-Mathfloat.FloorToInt(Mathfloat.Log10(Mathfloat.Abs(minDifference))), 0, kMaxDecimals);
        }
        ...
        {
            int decimals = Mathfloat.Clamp((int)(5 - Mathfloat.Log10(Mathfloat.Abs(v))), 0, kMaxDecimals);
